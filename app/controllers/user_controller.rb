...
class UserController < ApplicationController
  ...
  def login
    ...
    login_user(user)
    render_json status: 'ok', user: user.decorate.gui_hash
  end

  def follow
    return render_json status: 'User does not exist.' unless User.exist?(params[:username])
    # TODO: define context for this use case
  ...
  def username
    if logged_in?
      ...
      return render_json status: 'User does not exist.' if user.nil?
      return render_json status: 'User account has been disabled.' if user.status != 'active' || user.password.nil?
      return render_json status: 'ok', user: user.decorate.gui_hash
    end
  ...
  def new
    ...
    #user.save
    # TODO: this completely needs a context
    render_json status: 'ok'
  end
